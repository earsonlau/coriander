...
func main() {
	...
	//8、启动服务（优雅关机）
	//上面代码虽然用到gin来创建路由，但是实际上最终还是用go/http的方法来自定义server和启动server，
	//因为优雅关机是 http.Server内置的方法
	...
	//8.2、开启一个goroutine启动server，为什么要开协程去执行？
	//因为ListenAndServe执行时会不停循环监听请求路径，导致程序无法往下执行
	go func() {
